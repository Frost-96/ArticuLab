...
export const startSpeakingSchema = z
  .object({
    ...
    aiRole: z
      ...
      .max(200, "AI role must not exceed 200 characters")
      .optional(),
  })
  .refine(
    (data) => {
      ...
      const hasCustomParams =
        !!data.scenarioCategory && !!data.title && !!data.aiRole;
    ...
    {
      message:
        "Either provide scenarioId alone, or provide scenarioCategory, title, and aiRole together",
    },
  );
...
// Chat API 响应（包含 AI 回复和 TTS 音频）
export const speakingChatResponseSchema = z.object({
  userMessage: z.object({ id: z.string(), content: z.string() }),
  aiMessage: z.object({
    id: z.string(),
    content: z.string(),
    audioUrl: z.string().nullable(),
  }),
  /** TTS 音频的 Base64 编码（现阶段直接返回，后续改为 URL） */
  audioBase64: z.string().nullable(),
  totalTurns: z.number().int().min(0),
});

// 音频存储结果（预留接口，后续接入 Supabase Storage 时使用）
export const audioStorageResultSchema = z.discriminatedUnion("ok", [
  z.object({ ok: z.literal(true), url: z.string(), storageId: z.string() }),
  z.object({ ok: z.literal(false), error: z.string() }),
]);

// ==================== 发音评估 ====================

// POST /api/speaking/pronunciation 元数据校验（音频文件通过 FormData 传递）
export const pronunciationRequestSchema = z.object({
  language: z.string().default("en-US"),
  referenceText: z.string().min(1, "referenceText cannot be empty"),
  messageId: idSchema,
});

// 发音评估音素结果
export const pronunciationPhonemeResultSchema = z.object({
  phoneme: z.string(),
  accuracyScore: z.number(),
});

// 发音评估逐词结果（包含该单词的音素评分数组）
export const pronunciationWordResultSchema = z.object({
  word: z.string(),
  accuracyScore: z.number(),
  errorType: z.string(),
  phonemes: z.array(pronunciationPhonemeResultSchema),
});
...
export type SpeakingChatResponse = z.infer<typeof speakingChatResponseSchema>;
export type AudioStorageResult = z.infer<typeof audioStorageResultSchema>;

// 发音评估类型
export type PronunciationWordResult = z.infer<
  typeof pronunciationWordResultSchema
>;
export type PronunciationPhonemeResult = z.infer<
  typeof pronunciationPhonemeResultSchema
>;
