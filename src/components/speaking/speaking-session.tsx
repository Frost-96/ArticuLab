...
async function parseErrorResponse(response: Response) {
    try {
        ...
        return (
            result.error || "Speaking AI service is temporarily unavailable."
        );
    } catch {
...
export function SpeakingSession({ exercise }: SpeakingSessionProps) {
    ...
    const playNextStreamAudio = useCallback(() => {
        ...
        try {
            const url = audioBase64ToUrl(
                nextAudio.audioBase64,
                nextAudio.format,
            );
                    ...
                    prev.map((m) =>
                        m.id === messageId
                            ? {
                                  ...m,
                                  pronunciationScore:
                                      result.data!.pronunciationScore,
                                  pronunciationAccuracy:
                                      result.data!.accuracyScore,
                                  pronunciationFluency:
                                      result.data!.fluencyScore,
                                  pronunciationCompleteness:
                                      result.data!.completenessScore,
                                  pronunciationProsody:
                                      result.data!.prosodyScore,
                              }
                            : m,
                    ),
                );
                ...
                <div className="mx-auto flex max-w-4xl items-center justify-between gap-4">
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                        ...
                        {canFinish ? (
                            ...
                            >
                                <LoadingLink
                                    href={`/speaking/${exercise.id}/review`}
                                >
                        ...
                        {messages.length > 0 ? (
                            ...
                            <div className="flex flex-1 items-center justify-center">
                                <div className="max-w-md text-center text-sm leading-6 text-slate-500">
                                    No transcript yet. Start recording or type a
                                    response to begin the conversation.
                                </div>
                            </div>
                        )}
