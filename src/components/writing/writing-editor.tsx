...
function writeStopwatchSnapshot(
  exerciseId: string,
  state: WritingStopwatchState,
) {
  if (typeof window === "undefined") {
    return;
  }
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      writeStopwatchSnapshot(exerciseId, {
        elapsedSeconds:
          decodeStopwatchState(readStopwatchSnapshot(exerciseId, readOnly))
            .elapsedSeconds + 1,
        isRunning: true,
      });
    }, 1000);
...
export function WritingEditor({ exercise }: WritingEditorProps) {
  ...
  const persistDraft = useCallback(
    async (nextContent: string, silent = false) => {
      if (readOnly) {
        return true;
      }
      ...
      if (!result.success || !result.data) {
        setSaveError(!result.success ? result.error : "Failed to save draft.");
        if (!silent) {
  ...
  async function handleComplete() {
    ...
    try {
      const res = await fetch("/api/writing/submit", {
        ...
        body: JSON.stringify({
          exerciseId: exercise.id,
          scenarioType: exercise.scenarioType,
          isCustomPrompt: exercise.isCustomPrompt,
