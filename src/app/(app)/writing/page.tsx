...
import { type ExamType, WRITING_STATUS_CONFIG } from "@/types";
...
export default function Page() {
  ...
  const [examType, setExamType] = useState<ExamType>("ielts-task2");
  ...
  const currentExam =
    EXAM_CONFIGS.find((e) => e.id === examType) ?? EXAM_CONFIGS[0]!;

  const startSessionWithId = (
    prompt: string,
    sessionMode: "daily" | "exam" = mode,
  ) => {
    ...
    setCurrentWritingSession({
      ...
      title,
      mode: sessionMode,
      examType: sessionMode === "exam" ? examType : undefined,
      prompt,
      content: "",
      wordCount: 0,
      timeSpent: 0,
  ...
  const handleRandom = () => {
    const pool = mode === "exam" ? currentExam.prompts : DAILY_PROMPTS;
    const prompt = customPrompt || pickRandom(pool) || "";
    if (!prompt) return;
    startSessionWithId(prompt, mode);
  };
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      {/* Mode Tabs */}
      <Tabs value={mode} onValueChange={(v) => setMode(v as "daily" | "exam")}>
        ...
        <TabsContent value="daily" className="mt-4 space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-sm text-slate-500">
              Pick a prompt or get a random one to start your session.
            </p>
            <Button
              variant="outline"
              size="sm"
          ...
          {DAILY_PROMPTS.slice(0, 3).map((prompt, i) => (
            <PromptCard
              ...
              onClick={() => startSessionWithId(prompt, "daily")}
            />
          ))}
        </TabsContent>
        ...
        <TabsContent value="exam" className="mt-4 space-y-5">
              ...
              {EXAM_CONFIGS.map((exam) => (
                <button
                  ...
                  className={cn(
                    ...
                    examType === exam.id
                      ? cn(exam.pillActive, "border-transparent shadow-sm")
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-sm text-slate-500">Sample questions</p>
              <Button
                variant="outline"
                size="sm"
                className="gap-1.5"
            ...
            {currentExam.prompts.map((prompt, i) => (
              <PromptCard
                ...
                onClick={() => startSessionWithId(prompt, "exam")}
              />
            ))}
          </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-1 p-2">
            {writingSessions.map((session) => {
              const examCfg = session.examType
                ? EXAM_CONFIGS.find((e) => e.id === session.examType)
                : null;
              const statusCfg = WRITING_STATUS_CONFIG[session.status];
              return (
                <button
                  key={session.id}
                  onClick={() => {
