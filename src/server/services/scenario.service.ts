...
import type {
  ScenarioPrompt,
  ScenarioListResult,
} from "@/types/scenario/scenarioTypes";
import { getFirstError } from "@/lib/error";

type FindScenariosResult = Awaited<
  ReturnType<typeof scenarioRepo.findScenarios>
>;

function formatScenarioList(
  rows: FindScenariosResult["rows"],
): ScenarioPrompt[] {
...
function formatScenarioDetail(
  scenario: NonNullable<
    Awaited<ReturnType<typeof scenarioRepo.findScenarioById>>
  >,
): ScenarioDetail {
  return {
    id: scenario.id,
...
export async function getScenarioList(
  ...
  const { category, scenarioType, difficulty, page, pageSize } =
    parsedParams.data;
